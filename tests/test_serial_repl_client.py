...
def test_configure_wifi(client):
    ssid = WIFI_CREDENTIALS['ssid']
    psk = WIFI_CREDENTIALS['psk']

    ip = client.configure_wifi(ssid, psk)
    assert client.eval('sta_if.config("essid")') == ssid
    assert client.eval('sta_if.isconnected()')
    assert ping(ip)


def test_configure_webrepl(client):
    ssid = WIFI_CREDENTIALS['ssid']
    psk = WIFI_CREDENTIALS['psk']

    ip = client.configure_wifi(ssid, psk)

    client.configure_webrepl('password')
